...
export const Sidebar = ({ items = [{ label: "Home", link: "/" }] }) => {
    ...
    return (
        <div
            ...
            <nav
                className={`fixed top-0 left-0 h-full bg-white shadow-lg w-auto transform transition-transform duration-300 ${toggleViewSidebar ? "translate-x-0" : "-translate-x-full"
                    }`}
                ...
                <ul className="mt-16 flex flex-col gap-3 px-6">
                    {items.map((item, index) => (
                        <li
                            key={index}
                            className="flex gap-1 text-nowrap cursor-pointer rounded-md px-4 py-2 text-gray-700 hover:bg-gray-100 hover:text-gray-900 transition"
                            onClick={() => {
                                navigate(item.link);
